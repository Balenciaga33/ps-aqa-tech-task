...
/**
 * Product debt already tracked in FINDINGS.md — not a merge blocker.
 ...
 * Known product violations are annotated and allowed (see FINDINGS.md #7–#8).
 */
export async function expectNoUnexpectedSeriousA11yViolations(page: Page, label: string) {
  ...
  if (known.some((v) => v.id === 'color-contrast')) {
    annotateKnownIssue(7, 'Primary buttons fail WCAG AA color contrast');
  }
  if (known.some((v) => v.id === 'html-has-lang')) {
    annotateKnownIssue(8, 'Document <html> is missing lang attribute');
  }
